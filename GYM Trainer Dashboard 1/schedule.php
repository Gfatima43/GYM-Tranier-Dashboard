...
if ($isTrainer && $trainerId) {
    $stmt = $pdo->prepare("SELECT wp.*, COUNT(w.id) AS exercise_count FROM workout_plans wp LEFT JOIN workouts w ON w.plan_id=wp.id WHERE wp.trainer_id=? GROUP BY wp.id ORDER BY wp.id DESC");
    $stmt->execute([$trainerId]);
    $plans = $stmt->fetchAll(PDO::FETCH_ASSOC);
} else {
    $plans = $pdo->query("SELECT wp.*, COUNT(w.id) AS exercise_count FROM workout_plans wp LEFT JOIN workouts w ON w.plan_id=wp.id GROUP BY wp.id ORDER BY wp.id DESC")->fetchAll(PDO::FETCH_ASSOC);
}

// Exercises by plan
$allExercises = $pdo->query("SELECT * FROM workouts ORDER BY plan_id, id")->fetchAll(PDO::FETCH_ASSOC);
$exercisesByPlan = [];
foreach ($allExercises as $ex) $exercisesByPlan[$ex['plan_id']][] = $ex;

// Clients
if ($isTrainer && $trainerId) {
    $cStmt = $pdo->prepare("SELECT * FROM clients WHERE trainer_id=? ORDER BY firstName");
    $cStmt->execute([$trainerId]);
...
$scheduleRows = $pdo->query("
    SELECT cs.id, cs.id, cs.MON, cs.TUE, cs.WED, cs.THU, cs.FRI, cs.SAT, cs.SUN, cs.client_id, cs.plan_id,
           c.firstName, c.lastName, c.trainer_id,
           wp.plan_name
    FROM client_schedules cs
    JOIN clients c ON cs.client_id = c.id
    JOIN workout_plans wp ON cs.plan_id = wp.id
    ORDER BY c.firstName, FIELD(cs.id, cs.MON, cs.TUE, cs.WED, cs.THU, cs.FRI, cs.SAT, cs.SUN,'Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday')
...
$clientsJs = [];
foreach ($clients as $c) $clientsJs[] = ['id' => $c['id'], 'name' => htmlspecialchars($c['firstName'] . ' ' . $c['lastName'], ENT_QUOTES)];
$clientsJson = json_encode($clientsJs, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT);

$plansJs = [];
foreach ($plans as $p) $plansJs[] = ['id' => $p['id'], 'name' => htmlspecialchars($p['plan_name'], ENT_QUOTES)];
        ...
        <div class="section-card">
            ...
            <?php if (empty($scheduleByClient)): ?>
                <div class="empty-state">
                    ...
                    <p>No schedules yet.<br>Click <strong>Assign Schedule</strong> to get started.</p>
                </div>
            <?php else: ?>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><?php foreach ($sched['days'] as $day): ?><span class="day-badge"><?= $day ?></span><?php endforeach; ?></td>
                                    <td>
                                        <button class="action-btn edit" title="Edit" onclick="openScheduleModal(<?= $sched['plan_id'] ?>,<?= $cid ?>,<?= json_encode($sched['days']) ?>)"><i class="fas fa-pen"></i></button>
