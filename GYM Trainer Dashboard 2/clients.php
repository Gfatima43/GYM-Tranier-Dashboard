...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($first) || empty($email)) {
            ...
            if ($check->fetch()) {
                ...
                try {
                    ...
                    (firstName, lastName, email, phone, plan, progress, sessions, status, user_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->execute([$first, $last, $email, $phone, $plan, $progress, $sessions, $status, $user_id]);
    ...
    elseif ($action === 'edit') {
        ...
        if ($rawPass) {
            ...
            $pdo->prepare("UPDATE users SET password=? WHERE id=(SELECT user_id FROM trainers WHERE id=?)")
                ->execute([$hash, $id]);
        }
        if (empty($first) || empty($email) || $id < 1) {
        ...
        <div class="modal-box">
            ...
            <form method="POST" action="clients.php">
                ...
                <div class="form-row" style="margin-bottom:1.4rem">
                    ...
                    <div><label class="gym-label">Last Name *</label><input type="text" name="lastName" class="gym-input" placeholder="Last name"></div>
                </div>
