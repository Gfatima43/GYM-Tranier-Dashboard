...
<body>
                                    ...
                                    <tr>
                                        <td>
                                            <div style="display:flex;align-items:center;gap:1.2rem">
                                                <div class="client-av" style="background:<?= $bg ?>"><?= $initials ?></div>
                                        ...
                                        <td>
                                            <button class="action-btn edit" title="Edit"
                                                onclick="editScheduleModal(<?= $cid ?>, <?= $sched['plan_id'] ?>, '<?= htmlspecialchars($sched['name'], ENT_QUOTES) ?>', <?= htmlspecialchars(json_encode($sched['days']), ENT_QUOTES) ?>, <?= htmlspecialchars(json_encode($sched['ids']), ENT_QUOTES) ?>)">
                                                <i class="fas fa-pen"></i>
                                            </button>
                                            <button class="action-btn del" title="Remove"
                                                onclick="openDeleteScheduleModal(<?= $sched['ids'][0] ?>, '<?= htmlspecialchars($sched['name'], ENT_QUOTES) ?>')">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                        </td>
                                    </tr>
        ...
        <div class="modal-overlay" id="deleteScheduleModal">
            <div class="modal-box" style="max-width:44rem">
                ...
                <form method="POST" action="workout-plan.php" id="deleteSchedForm">
                    ...
                    <input type="hidden" name="schedule_id" id="deleteSchedId">
                    <div style="display:flex;gap:1rem">
    ...
    <script>
        ...
        function openScheduleModal(planId, clientId, currentDays) {
            ...
            document.getElementById('schedForm').removeAttribute('data-edit-mode');
            document.getElementById('schedForm').removeAttribute('data-schedule-ids');
            document.getElementById('clientSelectWrap').style.display = 'block';
